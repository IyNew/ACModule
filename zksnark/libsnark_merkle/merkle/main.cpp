...
bool verify_merkle_tree_proof(
    const r1cs_gg_ppzksnark_verification_key<ppT>& vk,
    const r1cs_gg_ppzksnark_proof<ppT>& proof,
    const libff::bit_vector& root)
{
    // Prepare the primary input
    r1cs_primary_input<FieldT> primary_input;
    primary_input.resize(root.size());
    for (size_t i = 0; i < root.size(); ++i)
    {
        primary_input[i] = FieldT(root[i]);
    }

    // Verify the proof
    return r1cs_gg_ppzksnark_verifier_strong_IC<ppT>(vk, primary_input, proof);
}
...
template<typename ppzksnark_ppT, typename FieldT, typename HashT>
boost::optional<r1cs_gg_ppzksnark_proof<ppzksnark_ppT>> generate_merkle_tree_proof(
    ...
    libff::bit_vector& root)
{
    ...
    // Initialize the Merkle tree circuit
    sample::MerkleCircuit<FieldT, HashT> mtc(pb, tree_depth, leaves);
    mtc.generate_r1cs_constraints();
    mtc.generate_r1cs_witness();

    // Check if the protoboard satisfies all constraints
    if (!pb.is_satisfied()) {
        std::cout << "pb is not satisfied" << std::endl;
        return boost::none;
    }

    // Extract the root from the protoboard
    root = pb.val(mtc.root_variable());
...
int main(int argc, char* argv[])
{
    ...
    libff::bit_vector root;
    std::vector<std::vector<libff::bit_vector>> levels(tree_depth + 1);
    int leaf_count = std::pow(2, tree_depth);

    // Generate witness
    // Level tree_depth leaves from leftmost to rightmost
    for (int i = 0; i < leaf_count; ++i) {
        // Assuming the leaves are provided via command-line arguments starting from argv[2]
        libff::bit_vector leaf = hash256<HashT>(argv[i + 2]);
        levels[tree_depth].push_back(leaf);
    }

    // Calculate all levels of the tree
    calcAllLevels<HashT>(levels, tree_depth);
    ...
    // Generate proof
    auto proof = generate_merkle_tree_proof<ppzksnark_ppT, FieldT, HashT>(
        pk, tree_depth, levels[tree_depth], root);
    ...
    // Save the root
    std::fstream root_file("root.txt", std::ios_base::out);
    root_file << *binToHex<HashT>(root);
    root_file.close();
}
    ...
    // Load root from command-line argument
    std::string r(argv[2]);
    libff::bit_vector root = hexToBin(r);

    // Verify the proof
    bool ret = verify_merkle_tree_proof<ppzksnark_ppT, FieldT>(vk, proof, root);
    if (ret) {
